...
StringRenderer::StringRenderer(int x, int y, const char * string) {
	this->x = x;
	this->y = y;

	renderString = string;

	allocLength = 8;//TODO Fix this initialization
...
void StringRenderer::rebuildData() {
	...
	int letterSize = ts::SpriteSheet::defaultFontSheet->getElementSizePixels() * 4;
	...
	for (unsigned int i = 0; i < renderString.size(); ++i) {
		...
		for (int k = 0; k < 4; ++k) {
			vertexData[letterVertexIndex + (k * 2)] = letterVertexData[(k * 2)] + (i * letterSize);
