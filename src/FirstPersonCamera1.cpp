...
#include "FirstPersonCamera1.h"

using namespace glm;

FirstPersonCamera1::FirstPersonCamera1() {
}

FirstPersonCamera1::~FirstPersonCamera1() {
}

void FirstPersonCamera1::update() {
	if (shouldUpdate) {
...
void FirstPersonCamera1::move(float x, float y, float z) {
	if (!(x == 0 && y == 0 && z == 0)) {
...
void FirstPersonCamera1::look(int dx, int dy) {
	if (!(dx == 0 && dy == 0)) {
		yaw = fmodf(yaw, 360);
		yaw += dx * lookSpeed;
